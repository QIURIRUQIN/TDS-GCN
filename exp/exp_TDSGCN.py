...
class Exp_TDSGCN(Exp_Basic):
    def __init__(self, args, isLoad=True):
        super(Exp_TDSGCN, self).__init__(args)
        self.args = args
        ...
        pos_inter = get_raw_data_from_file('pos_inter_TDSGCN', suffix='.pkl')
        pos_inter_timestamp = get_raw_data_from_file('pos_inter_timestamp_TDSGCN', suffix='.pkl')
        neg_inter = get_raw_data_from_file('neg_inter_TDSGCN', suffix='.pkl')
        neg_inter_timestamp = get_raw_data_from_file('neg_inter_timestamp_TDSGCN', suffix='.pkl')
        ...
        self.process_timestep(train_adj_time)
        self.initialize_model()
        self.model.record_data(
            pos_inter,
            neg_inter,
            pos_inter_timestamp,
            neg_inter_timestamp
        )
        del trainMat_p_n, pos_inter, neg_inter, pos_inter_timestamp, neg_inter_timestamp
        gc.collect()
    ...
    def process_timestep(self, train_adj_time):
        ...
        _, _, time_seq = coo_time.row, coo_time.col, coo_time.data
    ...
    def train(self):
        ...
        for user, item_p, item_n in train_loader:
            ...
            user_embedding, item_embedding, all_user_embedding, all_item_embedding = self.model(
    ...
    def val(self, dataloader, is_save=True):
        hit_ratio, NDCG = [], []
        user_embedding, item_embedding, _, _ = self.model(
            self.time_seq_tensor
        )
