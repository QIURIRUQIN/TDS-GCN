...
class KCGN(nn.Module):
    def __init__(self, args: dict, n_users: int, n_items: int, dims: List[int], maxTime: int, interactionNum: int=5):
        super(KCGN, self).__init__()
        self.args = args
        ...
        self.interactionNum = interactionNum
    ...
    def init_embedding(self):
        ...
        if self.use_multi_label:
            self.item_embd = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.interactionNum * self.n_items, self.hidden_dim)))
        else:
    ...
    def forward(self, graph, time_seq):
        ...
        if self.use_multi_label:
            item_embedding_mean = item_embedding.contiguous().view(self.n_items, self.interactionNum, self.hidden_dim * (len(self.layers) + 1))
